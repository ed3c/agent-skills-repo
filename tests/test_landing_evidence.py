...
def test_declared_paths_must_match_commit(tmp_path: Path) -> None:
    root, commit_sha = repository(tmp_path)
    manifest = completed_manifest(commit_sha)
    paths = ["README.md", "tests/test_landing_evidence.py"]
    landing = manifest["work_items"][0]["landing"]  # type: ignore[index]
    landing["changed_paths"] = paths
    landing["changed_paths_digest"] = changed_paths_digest(paths)

    with pytest.raises(LandingEvidenceError, match="changed_paths do not match commit"):
        validate_landing_evidence(manifest, SCHEMA, repo_root=root, main_ref="main")
