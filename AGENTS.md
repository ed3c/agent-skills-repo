...
- Flag any qualification claim without real physical evidence, cleanup proof, reproduction, and trusted verification.
- Flag any production/implicit routing change without explicit reviewed admission authority.
- Flag any Arena result that changes qualification state or omits failed runs.
- Flag direct generated-export edits without canonical-source and digest updates.
- Flag any secret leakage, unbounded network/production access, stale handoff, replay, trust-root ambiguity, or unsupported host/profile/policy.
